...
#       #self.pretrained_vgg16 = pretrained_vgg16
    ...
    def forward(self, batch_holistic, batch_header, batch_footer, batch_left_body, batch_right_body):
        #output_holistic = torch.tanh(self.pretrained_vgg16(batch_holistic))
#         output_header = self.pretrained_holistic(batch_header)
#         output_footer = torch.tanh(self.pretrained_holistic(batch_footer))
#         output_left_body = torch.tanh(self.pretrained_holistic(batch_left_body))
        output_right_body = torch.tanh(self.pretrained_holistic(batch_right_body))

        #output_all = torch.cat((output_holistic, output_header, output_footer, output_left_body, output_right_body), dim=-1)

        #ff1_out = torch.tanh(self.ff1(output_all))

        #ff2_out = torch.tanh(self.ff2(ff1_out))
